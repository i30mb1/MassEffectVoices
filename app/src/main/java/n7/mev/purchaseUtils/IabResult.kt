...
class IabResult(val response: Int, message: String?) {
    ...
    val isSuccess: Boolean
        get() = response == IabHelper.Companion.BILLING_RESPONSE_RESULT_OK
    ...
    init {
        if (message == null || message.trim { it <= ' ' }.length == 0) {
            this.message = IabHelper.Companion.getResponseDesc(response)
        } else {
            this.message = message + " (response: " + IabHelper.Companion.getResponseDesc(response) + ")"
        }
    }
}